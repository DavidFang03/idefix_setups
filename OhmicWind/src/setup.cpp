...
void Ambipolar(DataBlock &data, real t, IdefixArray3D<real> &xAin) {
    ...
    idefix_for(
        ...
        KOKKOS_LAMBDA(int k, int j, int i) {
            ...
            real B2 = Vc(BX1, k, j, i) * Vc(BX1, k, j, i) + Vc(BX2, k, j, i) * Vc(BX2, k, j, i)
...
void Resistivity(DataBlock &data, real t, IdefixArray3D<real> &etain) {
    ...
    idefix_for(
        ...
        KOKKOS_LAMBDA(int k, int j, int i) {
            ...
            // real EtaBuffer = epsilon*epsilon*0.05*FMAX((1.25*R0-r),0.0);
            // Transition across disk and corona (want eta to be zero outside the disk dead zone)
            real TransDC = 0.5 * (1 - tanh((fabs(zh) - Hideal) / (0.2 * trSmoothing)));
            // Transition across the DZI (want eta to be zero outside the disk dead zone)
            // real TransDZI = 0.5*(1+tanh((R - 10.0)/(0.1*trSmoothing)));
            // The expression for the magnetic Reynolds number (R_m) in the dead zone of the disk
            real RmDZ = RmDZ0 * 1 / (Vc(RHO, k, j, i) * Ri);
            // The expression for the Ohmic resistivity in the dead zone of the disk
            real etaDZ = pow(epsilon * Ri, 2) * Omega / RmDZ;
            // The final expression for the Ohmic resistivity (includes the buffer zone
            // contribution). Makes R_M = 50 at the inner edge of the DZI (feels appropriate - maybe
            // disk slightly heavy? - discuss) eta(k,j,i) =
            // (pow(Ri,1.5))*Vc(RHO,k,j,i)/(10.0*10.0*pow(10,0.5))*TransDC + EtaBuffer; eta(k,j,i) =
            // EtaBuffer + (pow(Ri,1.5))*Vc(RHO,k,j,i)/2500*TransDZI*TransDC; eta(k,j,i) = EtaBuffer
            // + etaDZ;
            eta(k, j, i)
    ...
    if ((dir == IDIR) && (side == left)) {
        ...
        hydro->boundary->BoundaryFor(
            "UserDefX1", dir, side, KOKKOS_LAMBDA(int k, int j, int i) {
                ...
                Vc(RHO, k, j, i)
                    = 1.0 / (Rin * sqrt(Rin))
                      * exp(1.0 / (csdisk * csdisk) * (1.0 / sqrt(Rin * Rin + z * z) - 1.0 / Rin));
                real densityFloor = computeDensityFloor(R, z, densityFloor0, Rin, epsilon);
                if (Vc(RHO, k, j, i) < densityFloor)
                    Vc(RHO, k, j, i) = densityFloor;
...
void EmfBoundary(DataBlock &data, const real t) {
    ...
    if (data.lbound[IDIR] == userdef) {
        ...
        idefix_for(
            "EMFBoundary", 0, data.np_tot[KDIR], 0, data.np_tot[JDIR], KOKKOS_LAMBDA(int k, int j) {
    ...
    if (data.lbound[JDIR] == axis) {
        ...
        idefix_for(
            "EMFBoundary", 0, data.np_tot[KDIR], 0, data.np_tot[IDIR], KOKKOS_LAMBDA(int k, int i) {
...
void Setup::InitFlow(DataBlock &data) {
    ...
    IdefixHostArray4D<real> A = IdefixHostArray4D<real>(
        "Setup_VectorPotential", 3, data.np_tot[KDIR], data.np_tot[JDIR], data.np_tot[IDIR]);
            ...
            for (int i = 0; i < d.np_tot[IDIR]; i++) {
                ...
                if (R > Rin) {
                    ...
                    if (std::isnan(d.Vc(VX3, k, j, i))) {
                        idfx::cout << "Nan in R>Rin at (i,j,k)=(" << i << "," << j << "," << k
                                   << "), (r,th,R,z)=(" << r << "," << th << "," << R << "," << z
                                   << ")" << std::endl;
                        IDEFIX_ERROR("Nan!s");
                    }
                    ...
                    d.Vc(VX3, k, j, i)
                        = 1.0 / sqrt(Rin)
                          * sqrt(FMAX(Rin / sqrt(Rin * Rin + z * z) - 2.5 * csdisk * csdisk, 0.0));
                    d.Vc(PRS, k, j, i) = cs2 * d.Vc(RHO, k, j, i);
                    if (std::isnan(d.Vc(VX3, k, j, i))) {
                        idfx::cout << "Nan in R<Rin at (i,j,k)=(" << i << "," << j << "," << k
                                   << "), (r,th,R,z)=(" << r << "," << th << "," << R << "," << z
