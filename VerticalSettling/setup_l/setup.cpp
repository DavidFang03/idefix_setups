...
void UserdefBoundaryParticles(DataBlock &data, real t, int dir, BoundarySide side) {
  ...
  if ((dir == KDIR) && (side == left)) {
    const real zl = data.mygrid->xbeg[KDIR];
    idefix_for(
        "UserdefParticlesBoundary", 0, capacity, KOKKOS_LAMBDA(int idx) {
          real x = states(dir, idx);
          if (isActive(idx) && x < zl) {
            isActive(idx) = false;
            Kokkos::atomic_add(&device_counter(0), 1);
          }
        });
  ...
  if ((dir == KDIR) && (side == right)) {
    const real zr = data.mygrid->xend[KDIR];
    idefix_for(
        "UserdefParticlesBoundary", 0, capacity, KOKKOS_LAMBDA(int idx) {
          real x = states(dir, idx);
          if (isActive(idx) && x > zr) {
            isActive(idx) = false;
            Kokkos::atomic_add(&device_counter(0), 1);
          }
        });
