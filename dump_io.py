...
class DumpDataset(object):
    ...
    def _read_fields(self, fh):
        ...
        if "PX" in self.data:
            self.data["PX"] = self.data["PX"].reshape(self.data["PMASS"].size, -1)
            dimensionality = self.data["PX"].shape[1]

            for i in range(dimensionality):
                # particle positions are set as views to avoid duplicating memory
                self.data[f"PX{i + 1}"] = self.data["PX"][:, i]
...
if __name__ == "__main__":
    print(
        readDump(
            "/home/dp316/dp316/dc-fang1/IdefixRuns/AODustyLWind/outputs/clean_wind_100_b1e4/dump.0015.dmp"
        ).data
    )
